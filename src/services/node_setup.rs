...
pub fn setup_node<H: HostApi>(
    ...
    let script_path = upload_installer(host, &node)?;
    let command = installation_command(&node, &input, &script_path)?;
...
fn upload_installer<H: HostApi>(host: &mut H, node: &NodeInfo) -> Result<String, ApiError> {
    let (filename, script) = match node.os_kind() {
        NodeOs::Linux => (
            "install-linux.sh",
    ...
    let relative_path = format!("{PLUGIN_DIR}/{filename}");
    host.upload(node.id, &relative_path, script, 0o700)?;

    absolute(node, &relative_path)
}
